...
class FeatureEngineer:
    ...
    def _create_recovery_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "hrv" in df.columns:
            ...
            df["hrv_7d_zscore"] = (df["hrv"] - df["hrv_7d_mean"]) / (
                df["hrv"].rolling(window=7, min_periods=1).std() + 1e-6
            )
            df["hrv_trend"] = (
                df["hrv"]
        ...
        if "sleep_duration_hours" in df.columns:
            df["sleep_debt"] = (
                ...
                .rolling(window=3, min_periods=1)
                .apply(lambda x: max(0, 8 - x.mean()) if len(x) > 0 else 0)
            )
            df["sleep_7d_mean"] = (
                df["sleep_duration_hours"].rolling(window=7, min_periods=1).mean()
            )

        # Readiness score
        if "readiness_score" in df.columns:
            df["readiness_7d_mean"] = (
                df["readiness_score"].rolling(window=7, min_periods=1).mean()
            )
            df["readiness_trend"] = (
                df["readiness_score"]
                .rolling(window=3, min_periods=1)
    ...
    def _create_load_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "active_calories" in df.columns:
            df["calories_7d_sum"] = (
                df["active_calories"].rolling(window=7, min_periods=1).sum()
            )
            df["calories_3d_sum"] = (
                df["active_calories"].rolling(window=3, min_periods=1).sum()
            )

        # Activity score
        if "activity_score" in df.columns:
            df["activity_7d_mean"] = (
                df["activity_score"].rolling(window=7, min_periods=1).mean()
            )
    ...
    def _create_consistency_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "completion" in df.columns:
            df["completion_7d_rate"] = (
                df["completion"].rolling(window=7, min_periods=1).mean()
            )
        ...
        if "completion" in df.columns:
            df["streak"] = (
                (df["completion"] == 1)
                .groupby((df["completion"] != 1).cumsum())
                .cumsum()
            )

        # Days since last training
        if "completion" in df.columns:
            df["days_since_training"] = df.groupby(
                (df["completion"] == 1).cumsum()
            ).cumcount()
