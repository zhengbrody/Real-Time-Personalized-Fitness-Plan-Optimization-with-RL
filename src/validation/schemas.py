...
class BodyState(BaseModel):
    ...
    readiness_score: int = Field(
        ..., ge=0, le=100, description="Oura readiness score (0-100)"
    )
    sleep_score: int = Field(..., ge=0, le=100, description="Oura sleep score (0-100)")
    hrv: int = Field(
        ..., ge=0, le=200, description="Heart Rate Variability in milliseconds"
    )

    # Apple Watch Metrics
    resting_hr: int = Field(..., ge=30, le=120, description="Resting heart rate in bpm")
    activity_score: int = Field(
        ..., ge=0, le=100, description="Daily activity score (0-100)"
    )
    exercise_minutes: int = Field(
        default=0, ge=0, le=300, description="Exercise minutes today"
    )

    # Subjective Metrics
    fatigue: int = Field(
        ..., ge=1, le=10, description="Subjective fatigue level (1=fresh, 10=exhausted)"
    )
    mood: Optional[int] = Field(
        default=5, ge=1, le=10, description="Mood score (1=very bad, 10=excellent)"
    )
    stress: Optional[int] = Field(
        default=5, ge=1, le=10, description="Stress level (1=very low, 10=very high)"
    )

    # Optional Advanced Metrics
    body_temperature: Optional[float] = Field(
        default=None, ge=35.0, le=42.0, description="Body temperature in Celsius"
    )
    ...
    def validate_resting_hr(cls, v: int) -> int:
        ...
        if v > 100:
            raise ValueError(
                "Resting HR above 100 may indicate issues. Consult a doctor."
            )
        return v
...
class WorkoutRecommendation(BaseModel):
    ...
    workout_type: WorkoutType = Field(..., description="Type of workout recommended")
    intensity: Intensity = Field(..., description="Recommended intensity level")
    duration_minutes: int = Field(
        ..., ge=0, le=180, description="Recommended workout duration in minutes"
    )
    explanation: Optional[str] = Field(
        default=None, description="Human-readable explanation for the recommendation"
    )
...
class TrainingEntry(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.now, description="When the training occurred"
    )
    state: BodyState = Field(..., description="Body state before the workout")
    recommendation: Optional[WorkoutRecommendation] = Field(
        default=None, description="The recommended workout"
    )
    ...
    actual_duration: Optional[int] = Field(
        default=None, ge=0, le=300, description="Actual workout duration in minutes"
    )
    source: DataSource = Field(
        default=DataSource.MANUAL_UPLOAD, description="Source of this entry"
    )
    user_feedback: Optional[str] = Field(
        default=None, description="User's feedback on the workout"
    )
...
class UserProfile(BaseModel):
    ...
    weight: float = Field(..., ge=30.0, le=300.0, description="User weight in kg")
    height: float = Field(..., ge=100.0, le=250.0, description="User height in cm")
    fitness_goal: FitnessGoal = Field(..., description="Primary fitness goal")
    training_experience: Optional[Literal["beginner", "intermediate", "advanced"]] = (
