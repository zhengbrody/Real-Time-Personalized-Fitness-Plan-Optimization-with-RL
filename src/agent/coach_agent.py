...
Be conversational, supportive, and data-driven. Use the user's body state data to inform your recommendations."""
    ...
    def process_daily_coaching(
        self, user_id: str, user_message: Optional[str] = None
    ) -> Dict:
        """
    ...
    def _adjust_plan_for_safety(
        self, plan: Dict, safety_result: SafetyCheckResult
    ) -> Dict:
    ...
    def _generate_daily_message(
        self, user_id: str, state: DailyState, plan: Dict
    ) -> Dict:
        """
    ...
    def _generate_conversational_response(
        self, user_id: str, user_message: str, state: DailyState, plan: Dict
    ) -> Dict:
        ...
        if self.llm_client:
            messages = [
                ...
                {
                    "role": "user",
                    "content": f"User state: {state.to_natural_language()}\n\nUser message: {user_message}",
                },
            ]
    ...
    def _process_llm_response(
        self, llm_response, user_id: str, state: DailyState, plan: Dict
    ) -> Dict:
        ...
        if message.tool_calls:
            for tool_call in message.tool_calls:
                ...
                if function_name == "adjust_plan":
                    ...
                    result = self.tools.explain_plan(
                        plan_id=plan["plan_id"], **function_args
                    )
                    tools_called.append({"tool": function_name, "result": result})
...
Use tools if you need to adjust the plan or set goals."""
    ...
    def _generate_rule_based_message(self, state: DailyState, plan: Dict) -> Dict:
        ...
        if state.readiness_score and state.readiness_score < 60:
            message_parts.append(
                "Your readiness score is lower today, so I've adjusted your plan to be lighter."
            )
        else:
            message_parts.append(
                "Based on your current body state, here's your recommended training plan."
            )

        # Add plan details
        message_parts.append(
            f"Today's plan: {plan.get('intensity', 'moderate')} intensity training."
        )

        # Motivation
        if state.motivation_level and state.motivation_level >= 7:
            message_parts.append(
                "Great to see your high motivation! Let's make the most of it."
            )
        elif state.motivation_level and state.motivation_level <= 4:
            message_parts.append(
                "I understand if you're not feeling super motivated today. Even a light session helps."
            )
    ...
    def _handle_safety_issue(
        self, state: DailyState, safety_result: SafetyCheckResult
    ) -> Dict:
