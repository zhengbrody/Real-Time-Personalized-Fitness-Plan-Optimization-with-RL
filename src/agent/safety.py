...
class SafetyGuardrails:
    ...
    def _check_physiological_signals(self, state: DailyState) -> SafetyCheckResult:
        ...
        if state.resting_heart_rate and state.resting_heart_rate > self.MAX_RESTING_HR:
            issues.append(
                f"Resting heart rate elevated ({state.resting_heart_rate} bpm)"
            )
    ...
    def _check_overtraining_risk(self, state: DailyState) -> SafetyCheckResult:
        ...
        if (
            state.training_frequency_last_week
            and state.training_frequency_last_week
            >= self.MAX_CONSECUTIVE_HIGH_INTENSITY
        ):
            return SafetyCheckResult(
                is_safe=False,
                risk_level="high",
    ...
    def _check_sleep_recovery(self, state: DailyState) -> SafetyCheckResult:
        ...
        if (
            state.sleep_duration_hours
            and state.sleep_duration_hours < self.MIN_SLEEP_HOURS
        ):
            return SafetyCheckResult(
                is_safe=False,
                risk_level="medium",
    ...
    def check_plan_safety(self, plan: Dict, state: DailyState) -> SafetyCheckResult:
        ...
        if (
            plan_intensity == "high"
            and state.fatigue_level
            and state.fatigue_level >= 7
        ):
            return SafetyCheckResult(
                is_safe=False,
                risk_level="high",
