...
from src.data_collection.apple_health import AppleHealthParser  # noqa: E402
...
class DataPreprocessor:
    ...
    def _aggregate_activity(self, activity_df: pd.DataFrame) -> pd.DataFrame:
        ...
        daily = (
            activity_df.groupby(["date", "type"]).agg({"value": "sum"}).reset_index()
        )
    ...
    def _aggregate_sleep(self, sleep_df: pd.DataFrame) -> pd.DataFrame:
        ...
        daily = sleep_df.groupby("date").agg({"duration": "sum"}).reset_index()
    ...
    def preprocess_pmdata(self, pmdata_dir: str) -> pd.DataFrame:
        ...
        participants = sorted(
            [d for d in pmdata_path.iterdir() if d.is_dir() and d.name.startswith("p")]
        )
    ...
    def unify_daily_data(
        ...
        unified = apple_df.merge(
            oura_df, on=["date", "user_id"], how="outer", suffixes=("_apple", "_oura")
        )

        # Merge PMData if available
        if pmdata_df is not None and not pmdata_df.empty:
            unified = unified.merge(
                pmdata_df, on=["date", "user_id"], how="outer", suffixes=("", "_pmdata")
            )
    ...
    def save_unified_data(
        self, unified_df: pd.DataFrame, filename: str = "unified_daily.parquet"
    ):
