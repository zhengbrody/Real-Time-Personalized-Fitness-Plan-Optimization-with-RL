...
class AppleHealthParser:
    ...
    def get_sleep_data(self) -> pd.DataFrame:
        ...
        if not sleep_df.empty:
            sleep_df["start_date"] = pd.to_datetime(
                sleep_df["start_date"], errors="coerce"
            )
            sleep_df["end_date"] = pd.to_datetime(sleep_df["end_date"], errors="coerce")
            sleep_df["duration"] = (
                sleep_df["end_date"] - sleep_df["start_date"]
    ...
    def get_workout_data(self) -> pd.DataFrame:
        ...
        if not workout_df.empty:
            workout_df["start_date"] = pd.to_datetime(
                workout_df["start_date"], errors="coerce"
            )
            workout_df["end_date"] = pd.to_datetime(
                workout_df["end_date"], errors="coerce"
            )
            workout_df["duration"] = (
                workout_df["end_date"] - workout_df["start_date"]
