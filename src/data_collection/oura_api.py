...
class OuraDataCollector:
    ...
    def __init__(self, personal_access_token: Optional[str] = None):
        ...
        if not token:
            raise ValueError(
                "Oura token required. Set OURA_TOKEN environment variable or pass token directly."
            )
    ...
    def get_sleep_data(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        """
        try:
            sleep_data = self.client.sleep_summary(
    ...
    def get_activity_data(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        """
        try:
            activity_data = self.client.daily_activity(
                start_date=start_date, end_date=end_date
            )
        except Exception as e:
    ...
    def get_readiness_data(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        """
        try:
            readiness_data = self.client.daily_readiness(
                start_date=start_date, end_date=end_date
            )
        except Exception as e:
    ...
    def sync_recent_data(
        self, days: int = 7, save_path: Optional[str] = None
