...
class OnlineLearningLoop:
    ...
    def __init__(
        self, recommender: Optional[HybridRecommender] = None, kafka_producer=None
    ):
        """
    ...
    def process_feedback(self, user_id: str, action_id: int, feedback: Dict) -> float:
        """
    ...
    def _log_event(self, event: Dict):
        ...
        if self.kafka_producer:
            try:
                ...
                topic = (
                    "training.user.feedback"
                    if "feedback" in event["event_type"]
                    else "training.plan.served"
                )
