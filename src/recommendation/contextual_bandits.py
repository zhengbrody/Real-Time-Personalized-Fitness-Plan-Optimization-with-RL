...
class ContextualBandit:
    ...
    def __init__(
        self, action_space: ActionSpace, reward_fn: Optional[RewardFunction] = None
    ):
        """
    ...
    def select_action(
        self, context: np.ndarray, allowed_actions: Optional[List[int]] = None
    ) -> int:
        """
    ...
    def get_action_probabilities(
        ...
        expected_rewards = {}
        for action_id in allowed_actions:
            expected = self.alpha[action_id] / (
                self.alpha[action_id] + self.beta[action_id]
            )
